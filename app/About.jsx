import React from "react";

function About() {
  return (
    <div className="flex flex-col items-center justify-center my-12 gap-6  ">
      <h1  className="text-center">
        About Me
      </h1>
      <p className=" text-center w-full md:w-1/2">
        I&apos;m a highly motivated problem-solver with a knack for innovative
        solutions. I thrive in cross-functional teams, adapting quickly to new
        tech and methodologies. Passionate about staying updated in the
        industry, I communicate technical information effectively to diverse
        audiences. Results-driven and committed to quality, I&apos;m an ideal
        candidate for challenging roles in tech.
      </p>
    </div>
  );
}

export default About;
